...
export class PaintGrid {
  ...
  applyPaintEvent(event: PaintEvent): void {
    ...
    // Apply main impact circle
    this.fillCircleUV(event.u, event.v, event.radius, team);
    ...
    for (const splat of splatters) {
      this.fillCircleUV(splat.u, splat.v, splat.radius, team);
    }
  }

  private fillCircleUV(centerU: number, centerV: number, radiusUV: number, newTeam: Team): void {
